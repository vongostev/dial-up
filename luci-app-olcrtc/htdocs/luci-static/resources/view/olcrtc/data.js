...
var callPingTunnel = rpc.declare({object: 'olcrtc-bot', method: 'ping_tunnel'});
...
return view.extend({
	...
	_renderMetrics: function (status) {
		...
		var stableNote = (running && status.uptime_seconds && status.uptime_seconds > 30)
			? ' (' + _('Stable > 30s') + ')' : '';

		var rows = [
			E('tr', {}, [E('td', {'width': '30%'}, [_('Process Status')]),
				E('td', {}, [statusEmoji(running), ' ', running ? _('Running') + stableNote : _('Stopped')])]),
			E('tr', {}, [E('td', {}, [_('Current Provider')]),
				E('td', {}, [prov.kind ? prov.kind + ' \u00b7 ' + prov.id : '\u2014'])]),
			E('tr', {}, [E('td', {}, [_('Active Transport')]),
				E('td', {}, [transport])])
		];
		...
		/* ── Metrics ────────────────────────────────────── */
		var rttSpan = E('span', {}, [self._lastRtt || '\u2014']);
		var metricRows = [
			E('tr', {}, [E('td', {'width': '30%'}, [_('Uptime')]), E('td', {}, [fmtUptime(status.uptime_seconds)])]),
			E('tr', {}, [E('td', {}, [_('Tunnel RTT')]), E('td', {}, [
				rttSpan, ' ',
				E('button', {
					'class': 'cbi-button cbi-button-action',
					'click': function () {
						rttSpan.textContent = _('measuring...');
						callPingTunnel().then(function (res) {
							self._lastRtt = res.error ? (_('Error:') + ' ' + res.error) : (res.rtt_ms + ' ms');
							rttSpan.textContent = self._lastRtt;
						}).catch(function (err) {
							self._lastRtt = _('Error:') + ' ' + (err.message || err);
							rttSpan.textContent = self._lastRtt;
						});
					}
				}, [_('Measure RTT')])
