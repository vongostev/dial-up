...
> [!WARNING]
> Проект находится в стадии бета-версии. Возможны ошибки и нестабильная работа. Протестировано на GLi.Net MT6000 (сервер), Cudy TR3000 (клиент, tethering)
...
Просто отправьте боту ссылку в формате:
...
Бот автоматически распознает провайдера, настроит оба роутера и поднимет туннель.
...
- При стабильном соединении (>30 сек) маршрут автоматически переключается на `proxy`.
- При обрыве туннеля — возврат на `direct` (интернет через оператора).
...
| Вкладка | Возможности |
|---|---|
| **Bot** | Роль (клиент/сервер), VK-токен, разрешённые пользователи, расширенные настройки |
| **Tunnel** | Ключ шифрования, провайдер, статус процесса, управление (start/stop/restart), upstream SOCKS5 (сервер) |
| **Network** | tproxy-статус, sing-box, firewall, whitelist-домены, маршрутизация, диагностика, пинг туннеля |
| **Logs** | Логи dial-up, olcrtc, sing-box с фильтрацией |
...
- [TCP-over-WebRTC туннель](https://github.com/openlibrecommunity/olcrtc)
