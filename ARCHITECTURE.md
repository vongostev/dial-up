...
├── luci-app-olcrtc/
│   ├── htdocs/.../view/olcrtc/
│   │   ├── bot.js                   # Вкладка Bot: роль, токен, настройки
│   │   ├── tunnel.js                # Вкладка Tunnel: ключ, провайдер, SOCKS5
...
UDP:53 (DNS) исключён из tproxy --- запросы LAN-клиентов идут в dnsmasq напрямую.
...
- **Outbounds**: `proxy` (SOCKS5 → 127.0.0.1:1080 → olcrtc), `direct` (с `domain_resolver: yandex`), `route-select` (selector)
- **Route rules**: whitelist → direct, остальное → route-select
- **DNS**: Yandex (77.88.8.8) для direct-резолвера, local для .lan/.internal
- **Без `default_domain_resolver`**: SOCKS outbound не резолвит домены, передаёт их как SOCKS5 ATYP=3 (domain) в olcrtc
- **Sniff с `override_destination: true`**: заменяет destination IP на сниффнутый домен (TLS SNI / HTTP Host), чтобы SOCKS outbound отправлял ATYP=3 (domain), а не IP. Без этого флага sing-box только обогащает метаданные для маршрутизации, но destination остаётся IP.
...
4 вкладки, rpcd-бэкенд (POSIX shell, 14 ubus-методов), JS-views.
...
| Метод | Описание |
|---|---|
| `get_status` | Статус из Go statusapi (fallback: syslog) — включает `tunnel_rtt_ms` |
...
> **RTT туннеля**: больше не используется отдельный rpcd-метод `ping_tunnel`.
> Замер RTT выполняется в Go-бэкенде через `singbox.Controller.DelayTest()` (Clash API
> `/proxies/proxy/delay`) и кэшируется в фоновом `netCache` (обновление каждые 30 с).
> Значение доступно через `Status.TunnelRTTMs` → `/status` JSON → `get_status` ubus.
...
### Доменная маршрутизация через туннель

Ключевая особенность: **клиентский sing-box не резолвит домены** для SOCKS outbound. Домены передаются как SOCKS5 ATYP=3 (domain name) через туннель на сервер, где их резолвит podkop через свой dnsmasq → sing-box → FakeIP.

Это позволяет серверу применять собственную доменную маршрутизацию (FakeIP, списки podkop) к трафику из туннеля:

```mermaid
graph LR
    subgraph client["КЛИЕНТ"]
        LAN["LAN-устройство"] --> DNS1["dnsmasq<br/>(резолв для LAN)"]
        LAN --> NFT["nft tproxy → sing-box"]
        NFT -->|"sniff SNI → домен"| SB["SOCKS outbound<br/>без domain_resolver"]
        SB -->|"ATYP=3 domain"| OC["olcrtc client"]
    end

    OC -->|"WebRTC туннель<br/>(передаёт домен)"| OS["olcrtc server"]

    subgraph server["СЕРВЕР"]
        OS -->|"dial(domain:port)"| PKP["podkop / sing-box<br/>dnsmasq:53 → FakeIP"]
        PKP -->|"домен в списке"| AWG["awg0 (VPN)"]
        PKP -->|"домен НЕ в списке"| DIR["direct (провайдер)"]
    end
```

- **Whitelist-домены** (VK, Яндекс и др.) → маршрутизируются через `direct` на клиенте (с `domain_resolver: yandex`), в туннель не попадают
- **Остальные домены** → передаются как ATYP=3 в туннель → сервер резолвит через `127.0.0.1:53` → FakeIP для доменов из списков podkop (twitter, telegram, cloudflare и др.) → awg0
